...
function getNextFriday(date) {
  ...
  const today = newDate.getUTCDay();
  if (today >= 5) newDate.setUTCDate(newDate.getUTCDate() + 7);
  newDate.setUTCDate(newDate.getUTCDate() + 5 - today);
  return newDate;
}
...
function getWeekNumberByDate(date) {
  ...
  const startOfTheYear = new Date(currentDate.getUTCFullYear(), 0, 1);
  const answerMs =
    (currentDate.getTime() - startOfTheYear.getTime()) / 86400000;
  const answerDays = answerMs + startOfTheYear.getDay();
  return Math.ceil(answerDays / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  const newDate = new Date(startYear, startMonth, 13);
  for (;;) {
